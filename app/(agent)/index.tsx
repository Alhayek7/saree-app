...
export default function AgentHomeScreen() {
  ...
  const { user } = useAuth();
  const { sales, stats, getBalance, checkLowBalance } = useAgent(); // ✅ تم دمج الاستيرادات
  ...
  return (
    ...
    >
      ...
      {/* ✅ تحذير الرصيد المنخفض - جديد */}
      {isLowBalance && (
        <TouchableOpacity 
          style={[styles.lowBalanceWarning, { backgroundColor: "#FFF8E1", borderColor: "#FFA63D" }]}
          onPress={() => router.push("/(agent)/recharge")}
          activeOpacity={0.9}
        >
          <Feather name="alert-triangle" size={18} color="#FFA63D" />
          <Text style={styles.lowBalanceText}>
            ⚠️ رصيدك على وشك النفاذ! متبقي {balance.availableOrders} طلب فقط
          </Text>
          <Feather name="chevron-left" size={16} color="#FFA63D" />
        </TouchableOpacity>
      )}
      ...
      <View style={{ paddingHorizontal: 16, marginTop: 8 }}>
        ...
        <View style={styles.actionsGrid}>
          {[
            ...
            { label: "شحن رصيد", icon: "refresh-cw" as const, color: AGENT_COLOR, route: "/(agent)/recharge", desc: "طلب شحن رصيد" },
...
const styles = StyleSheet.create({
  ...
  // ✅ تحذير الرصيد المنخفض - جديد
  lowBalanceWarning: { flexDirection: "row", alignItems: "center", gap: 10, marginHorizontal: 16, marginTop: 12, marginBottom: 4, padding: 12, borderRadius: 12, borderWidth: 1 },
  lowBalanceText: { flex: 1, fontSize: 13, fontWeight: "600", color: "#856404" },

  todayWrap: { flexDirection: "row", gap: 10, paddingHorizontal: 16, marginTop: 8, marginBottom: 12 },
...
ordersBalanceHeader: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 8 },
ordersBalanceLabel: { fontSize: 12 },
ordersBalanceValue: { fontSize: 32, fontWeight: "800" },
ordersBalanceUnit: { fontSize: 11, marginTop: 4 },
ordersRechargeBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, marginTop: 10 },
ordersRechargeText: { color: "#fff", fontSize: 12, fontWeight: "600" },
